...
class ArModelBuilder {
    ...
    /* Creates a node from a given image path or URL. The image asset loading in Sceneform is asynchronous, so the function returns a compleatable future of type Node */
    @RequiresApi(Build.VERSION_CODES.N)
    fun makeNodeFromImage(context: Context, transformationSystem: TransformationSystem, objectManagerChannel: MethodChannel, enablePans: Boolean, enableRotation: Boolean, name: String, modelPath: String, transformation: ArrayList<Double>): CompletableFuture<CustomTransformableNode> {
        val completableFutureNode: CompletableFuture<CustomTransformableNode> = CompletableFuture()
        val imageNode = CustomTransformableNode(transformationSystem, objectManagerChannel, enablePans, enableRotation)
        try {
            val imageView = ImageView(context).apply {
                val assetManager = context.assets
                val file = assetManager.open(modelPath)
...
//                        renderable.setSizer { view ->
//                            Vector3(0.2f, 0.2f, 0.2f) }
                        imageNode.name = name
                        val transform = deserializeMatrix4(transformation)
                    imageNode.worldScale = transform.first
                    imageNode.worldPosition = transform.second.apply {
                        z += transform.first.z / 2
                    }
                        //rotate the image node to 270 degrees in the z-axis
                        imageNode.worldRotation = transform.third.let {
                            Quaternion.multiply(it, Quaternion.axisAngle(Vector3(1f, 0f, 0f), 270f))
                        }
            ...
            completableFutureNode.completeExceptionally(e)
        }
//        ModelRenderable.builder()
//                .setSource(context, io.carius.lars.ar_flutter_plugin_flutterflow.R.raw.augmented_video_model)
//                .build().thenAccept{ renderable ->
//                    imageNode.renderable = renderable
//                    renderable.isShadowCaster = false
//                    renderable.isShadowReceiver = false
//                    var image = ImageView(context).apply {
//                        setImageResource(io.carius.lars.ar_flutter_plugin_flutterflow.R.drawable.car)
//                        scaleType = ImageView.ScaleType.FIT_CENTER
//                        adjustViewBounds = true
//                        layoutParams = android.view.ViewGroup.LayoutParams(1, 1)
//                    }
//                    imageNode.renderable = ViewRenderable.builder()
...
//                }
    ...
    // Creates a node from a given video path or URL. The video asset loading in Sceneform is asynchronous, so the function returns a compleatable future of type Node
    fun makeNodeFromVideo(context: Context, transformationSystem: TransformationSystem, objectManagerChannel: MethodChannel, enablePans: Boolean, enableRotation: Boolean, name: String, modelPath: String, transformation: ArrayList<Double>): CompletableFuture<ModelRenderable> {
        val completableFutureNode: CompletableFuture<ModelRenderable> = CompletableFuture()
        ModelRenderable.builder()
                .setSource(context, io.carius.lars.ar_flutter_plugin_flutterflow.R.raw.augmented_video_model)
                .build().thenAccept{ renderable ->
                    completableFutureNode.complete(renderable)
                }
                .exceptionally{throwable ->
                    completableFutureNode.completeExceptionally(throwable)
                    null // return null because java expects void return (in java, void has no instance, whereas in Kotlin, this closure returns a Unit which has one instance)
    ...
    // Creates a node from a given chroma key video path or URL. The chroma key video asset loading in Sceneform is asynchronous, so the function returns a compleatable future of type Node
    @RequiresApi(Build.VERSION_CODES.N)
    fun makeNodeFromChromaKeyVideo(context: Context, transformationSystem: TransformationSystem, objectManagerChannel: MethodChannel, enablePans: Boolean, enableRotation: Boolean, name: String, modelPath: String, transformation: ArrayList<Double>): CompletableFuture<ModelRenderable> {


        val completableFutureNode: CompletableFuture<ModelRenderable> = CompletableFuture()
        ModelRenderable.builder()
                .setSource(context, io.carius.lars.ar_flutter_plugin_flutterflow.R.raw.chroma_key_video_model)
                .build().thenAccept{ renderable ->
                    completableFutureNode.complete(renderable)
                }
                .exceptionally{throwable ->
