...
internal class AndroidARView(
    ...
    private var nodeAdded = true

    private val anchorNodeMap = mutableListOf<String>()
                            ...
                            if (anchorType != null){
                                when(anchorType) {
                                    0 -> { // Plane Anchor
                                        ...
                                        if ( name != null && transform != null && !anchorNodeMap.contains(name)){
                                            result.success(addPlaneAnchor(transform, name))
                                            anchorNodeMap.add(name)
                                        } else {
                                            result.success(false)
                                        }
                                    }
                                    1 -> { // Image Anchor
                                        val transform: ArrayList<Double>? = call.argument<ArrayList<Double>>("transformation")
                                        val name: String? = call.argument<String>("name")
                                        val image: String? = call.argument<String>("image")
                                        if (name != null && transform != null && image != null && !anchorNodeMap.contains(name)){
                                            result.success(addImageAnchor(transform, name, image))
                                            anchorNodeMap.add(name)
                                        } else {
                                            result.success(false)
                                        }
    ...
    private fun addImagesToDatabase(imagePaths: List<String>?) {
        if (imagePaths != null) {
            imageDatabase = AugmentedImageDatabase(arSceneView.session)
            for (image in imagePaths) {
                val loader: FlutterLoader = FlutterInjector.instance().flutterLoader()
                val key: String = loader.getLookupKeyForAsset(image)
                val bitmap = BitmapFactory.decodeStream(viewContext.assets.open(key))
...
//                        onDestroy()
    ...
    private fun initializeARView(call: MethodCall, result: MethodChannel.Result) {
        ...
        val argImageDatabase: List<String>? = call.argument<List<String>>("imageDatabase")
        ...
        addImagesToDatabase(argImageDatabase)
...
//                             Use getTrackingMethod() to determine whether the image is currently being tracked by the camera.
                            when (img.trackingMethod) {
                                ...
                                TrackingMethod.FULL_TRACKING -> {
                                    ...
                                    if (nodeAdded) {
                                        shouldUpdateAnchorPose(img)
                                        var args = HashMap<String, Any>()
                                        args["name"] = img.name
                                        args["pose"] = serializePose(img.centerPose)
                                        sessionManagerChannel.invokeMethod("onAugmentedImageDetected", args)
                                    }
                                }

                                TrackingMethod.NOT_TRACKING -> {
                    ...
                    if (::videoAnchorNode.isInitialized && :: videoRenderable.isInitialized && :: externalTexture.isInitialized) {
                    ...
                    fullTrackingImages.firstOrNull()?.let { augmentedImage ->
                        try {
                            ...
                            Log.e(TAG, "Could not play video [${videoAnchorNode.videoName}]", e)
                        }
                    }
                }
                }
...
// Get path to given Flutter asset
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        modelBuilder.makeNodeFromImage(viewContext, transformationSystem, objectManagerChannel, enablePans, enableRotation, dict_node["name"] as String, key, dict_node["transformation"] as ArrayList<Double>)
                                .thenAccept{node ->
                                    val anchorName: String? = dict_anchor?.get("name") as? String
                                    val anchorType: Int? = dict_anchor?.get("type") as? Int
                                    if (anchorName != null && anchorType != null) {
                ...
                5 -> { // Video
                    ...
                    val loader: FlutterLoader = FlutterInjector.instance().flutterLoader()
                    val key: String = loader.getLookupKeyForAsset(dict_node["uri"] as String)
                    val modelPath = "flutter_assets/assets/augmented_video_model.sfb"

                    modelBuilder.makeNodeFromVideo(viewContext, transformationSystem, objectManagerChannel, enablePans, enableRotation, dict_node["name"] as String, modelPath, dict_node["transformation"] as ArrayList<Double>)
...
//                                    arSceneView.scene.addChild(node)
                ...
                6 -> { // Chroma Key Video
                    ...
                    val loader: FlutterLoader = FlutterInjector.instance().flutterLoader()
                    val key: String = loader.getLookupKeyForAsset(dict_node["uri"] as String)
                    val modelPath = "flutter_assets/assets/chroma_key_video_model.sfb"

                    modelBuilder.makeNodeFromChromaKeyVideo(viewContext, transformationSystem, objectManagerChannel, enablePans, enableRotation, dict_node["name"] as String, modelPath, dict_node["transformation"] as ArrayList<Double>)
                            .thenAccept{renderable ->
                                videoRenderable = renderable
                                renderable.isShadowCaster = false
                                renderable.isShadowReceiver = false
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                                    renderable.material.setExternalTexture("videoTexture", externalTexture)
                                    renderable.material.setFloat4("keyColor", Color(0.278f, 1f, 0.145f, 1f))
                                }
                                val anchorName: String? = dict_anchor?.get("name") as? String
                                val anchorType: Int? = dict_anchor?.get("type") as? Int
                                if (anchorName != null && anchorType != null) {
...
//                                    arSceneView.scene.addChild(node)
                    ...
                    videoAnchorNode = VideoAnchorNode().apply {
                        videoName = key
                        videoScaleXYZ = deserializeMatrix4(dict_node["transformation"] as ArrayList<Double>).first
                        setParent(arSceneView.scene)
                    }
                }
    ...
    private fun addImageAnchor(transform: ArrayList<Double>, name: String, image: String): Boolean {
        return try {
            val position = floatArrayOf(deserializeMatrix4(transform).second.x, deserializeMatrix4(transform).second.y, deserializeMatrix4(transform).second.z)
            val rotation = floatArrayOf(deserializeMatrix4(transform).third.x, deserializeMatrix4(transform).third.y, deserializeMatrix4(transform).third.z, deserializeMatrix4(transform).third.w)
            val anchor: Anchor = arSceneView.session!!.createAnchor(Pose(position, rotation))
    ...
    private fun shouldUpdateAnchorPose(img: AugmentedImage) {
        if (anchorNodeMap.contains(img.name)) {
            val anchorNode = arSceneView.scene.findByName(img.name) as AnchorNode?
            if (anchorNode != null) {
                val pose = img.centerPose
                val anchor = anchorNode.anchor
                if (anchor != null) {
                    val oldPose = anchor.pose
                    val difference = oldPose.inverse().compose(pose)
                    if (difference.tx() > 0.1 || difference.ty() > 0.1 || difference.tz() > 0.1 || difference.qx() > 0.1 || difference.qy() > 0.1 || difference.qz() > 0.1 || difference.qw() > 0.1) {
                        anchor.detach()
                        anchorNode.anchor = arSceneView.session!!.createAnchor(pose)
                    }
    ...
    private fun playbackArVideo(augmentedImage: AugmentedImage,videoFixedXYZ : Vector3? = null) {
        Log.d(TAG, "playbackVideo = ${videoAnchorNode.videoName}")

        viewContext.assets.openFd(videoAnchorNode.videoName)
                .use { descriptor ->
                    ...
                    var videoScaleType = VideoScaleType.CenterCrop
                    if (videoAnchorNode.videoScaleXYZ != null) {
                        videoScaleType = VideoScaleType.FixSize
                    }
