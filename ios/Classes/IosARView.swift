...
class IosARView: NSObject, FlutterPlatformView, ARSCNViewDelegate, UIGestureRecognizerDelegate, ARSessionDelegate {
    ...
    private var trackedImages: Array<ARImageAnchor> = []
        ...
        switch call.method {
            ...
            case "addAnchor":
                if let type = arguments!["type"] as? Int {
                    ...
                    case 1: //Image Anchor
                        //NOT NEEDED ?
                        if let transform = arguments!["transformation"] as? Array<NSNumber>, let name = arguments!["name"] as? String, let image = arguments!["image"] as? String {
                            addImageAnchor(transform: transform, name: name, image: image)
                            result(true)
                        }
                        result(false)
                        result(true)
    ...
    func initializeARView(arguments: Dictionary<String,Any>, result: FlutterResult){
        ...
        // Set Image detection
        if let imageDatabase = arguments["imageDatabase"] as? [String], imageDatabase.count > 0 {
            var arImageSet = Set<ARReferenceImage>()
            //Add each trigger image
            for imagePath in imageDatabase {
                if let image = modelBuilder.getImageFrom(flutterAssetFile: imagePath){
                    let arImage = ARReferenceImage(image.cgImage!, orientation: CGImagePropertyOrientation.up, physicalWidth: 0.2)
                    arImage.name = imagePath
                    arImageSet.insert(arImage)
                }
            }
            configuration.detectionImages = arImageSet
            
            //This configuration gives better performances to image tracking
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        if let imageAnchor = anchor as? ARImageAnchor {
            self.trackedImages.append(imageAnchor)
            var dict = Dictionary<String, Any>()
            dict["name"] = imageAnchor.name
            dict["pose"] = serializePose(pose: anchor)
            DispatchQueue.main.async {
                DispatchQueue.main.async {self.sessionManagerChannel.invokeMethod("onAugmentedImageDetected", arguments: dict)}
            }
        }
    ...
    func renderer(_ renderer: SCNSceneRenderer, didUpdate node: SCNNode, for anchor: ARAnchor) {
        ...
        if let imageAnchor = anchor as? ARImageAnchor, let index = trackedImages.firstIndex(of: imageAnchor) {
            trackedImages[index] = imageAnchor
        }
    }
    ...
    func addNode(dict_node: Dictionary<String, Any>, dict_anchor: Dictionary<String, Any>? = nil) -> Future<Bool, Never> {
        return Future {promise in
                    ...
                    if let node: SCNNode = self.modelBuilder.makeNodeFromGltf(name: dict_node["name"] as! String, modelPath: key, transformation: dict_node["transformation"] as? Array<NSNumber>) {
                        if let anchorName = dict_anchor?["name"] as? String, let anchorType = dict_anchor?["type"] as? Int {
                            switch anchorType{
                                ...
                                case 1: //ImageDetection
                                if let anchor = self.trackedImages.first(where: { $0.referenceImage.name == anchorName }){
                                    self.sceneView.node(for: anchor)?.addChildNode(node)
                                    promise(.success(true))
                                }
                                default:
            ...
            case 4: // Image from Flutter asset folder
                // Get path to given Flutter asset
                let key = FlutterDartProject.lookupKey(forAsset: dict_node["uri"] as! String)
                // Add object to scene
                if let node: SCNNode = self.modelBuilder.makeNodeFromImage(name: dict_node["name"] as! String, modelPath: key, transformation: dict_node["transformation"] as? Array<NSNumber>) {
                    if let anchorName = dict_anchor?["name"] as? String, let anchorType = dict_anchor?["type"] as? Int {
                        switch anchorType{
                        case 0: //PlaneAnchor
                            if let anchor = self.anchorCollection[anchorName]{
                            ...
                            if let anchor = self.trackedImages.first(where: { $0.referenceImage.name == anchorName }){
                                // Attach node to the top-level node of the specified anchor
                                node.rotate(by: SCNQuaternion(x: 0, y: 1, z: 0, w: 0.5), aroundTarget: node.position)
                                node.localTranslate(by: SCNVector3(x: node.position.x/2, y: 0, z: 0))
                ...
                break
                case 5: // Video from WEB
                // Get path to given Flutter asset
                let key = FlutterDartProject.lookupKey(forAsset: dict_node["uri"] as! String)
                // Add object to scene
                if let node: SCNNode = self.modelBuilder.makeNodeFromVideo(name: dict_node["name"] as! String, modelPath: key, transformation: dict_node["transformation"] as? Array<NSNumber>) {
                    if let anchorName = dict_anchor?["name"] as? String, let anchorType = dict_anchor?["type"] as? Int {
                        switch anchorType{
                            case 0: //Video
                            if let anchor = self.trackedImages.first(where: { $0.referenceImage.name == anchorName }){
                ...
                break
                case 6: // ChromakeyVideo from WEB
                // Get path to given Flutter asset
                let key = FlutterDartProject.lookupKey(forAsset: dict_node["uri"] as! String)
                // Add object to scene
                if let node: SCNNode = self.modelBuilder.makeNodeFromChromaKeyVideo(name: dict_node["name"] as! String, modelPath: key, transformation: dict_node["transformation"] as? Array<NSNumber>) {
                    if let anchorName = dict_anchor?["name"] as? String, let anchorType = dict_anchor?["type"] as? Int {
                        switch anchorType{
                            case 1 : //ImageAnchor
                            if let anchor = self.trackedImages.first(where: { $0.referenceImage.name == anchorName }){
    ...
    //NOT NEEDED ?
    func addImageAnchor(transform: Array<NSNumber>, name: String, image: String){
        let arAnchor = ARAnchor(transform: simd_float4x4(deserializeMatrix4(transform)))
        anchorCollection[name] = arAnchor
        sceneView.session.add(anchor: arAnchor)
        // Ensure root node is added to anchor before any other function can run (if this isn't done, addNode could fail because anchor does not have a root node yet).
