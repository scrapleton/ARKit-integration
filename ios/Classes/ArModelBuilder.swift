...
class ArModelBuilder: NSObject {
    ...
    func getImageFrom(flutterAssetFile: String) -> UIImage? {
        let key = FlutterDartProject.lookupKey(forAsset: flutterAssetFile)
    ...
    // Creates a node from a given image model in the Flutter assets folder
    func makeNodeFromImage(name: String, modelPath: String, transformation: Array<NSNumber>?) -> SCNNode? {
        let material = SCNMaterial()
        let plane = SCNPlane()

        if let fileName = modelPath.components(separatedBy: "flutter_assets/").last, let image = getImageFrom(flutterAssetFile: fileName){
            material.diffuse.contents = image
            plane.materials = [material]
        }
        ...
        let planeNode = SCNNode(geometry: plane)
        if let transform = transformation {
            planeNode.transform = deserializeMatrix4(transform)
        }
        planeNode.eulerAngles.x = -.pi / 2
        planeNode.opacity = 1
        planeNode.name = name
    ...
    //Maybe usefull to load video from local folder
...
//    }
    ...
    // Creates a node from a given VIDEO model in the Flutter assets folder
    func makeNodeFromVideo(name: String, modelPath: String, transformation: Array<NSNumber>?) -> SCNNode? {
        let material = SCNMaterial()
        let plane = SCNPlane()
        guard let stringUrl = modelPath.components(separatedBy: "flutter_assets/").last else {
            return nil
        }
    ...
    // Creates a node from a given CHROMAKEYVIDEO model from WEB
    func makeNodeFromChromaKeyVideo(name: String, modelPath: String, transformation: Array<NSNumber>?) -> SCNNode? {
        let material = SCNMaterial()
        let plane = SCNPlane()
        guard let stringUrl = modelPath.components(separatedBy: "flutter_assets/").last else {
            return nil
        ...
        planeNode.geometry!.materials = [chromaKeyMaterial]
        return planeNode
    }
    
    //Used to loop videoplayer
