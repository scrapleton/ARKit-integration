...
typedef ARImageMarkerResultHandler = void Function(
    String imageMarker, Matrix4? pose);
...
class ARSessionManager {
  ...
  /// List of image markers that the AR session should track
  final List<String> imageMarkers;
  ...
  /// Receives the image marker that was detected
  late ARImageMarkerResultHandler onAugmentedImageDetected;

  /// Callback that is triggered once error is triggered
  ErrorHandler? onError;

  ARSessionManager(
      int id, this.buildContext, this.planeDetectionConfig, this.imageMarkers,
      {this.debug = false}) {
    ...
    try {
      switch (call.method) {
        ...
        case 'onAugmentedImageDetected':
          if (onAugmentedImageDetected != null) {
            var args = Map<String, dynamic>.from(call.arguments);
            onAugmentedImageDetected(
                args['name'], MatrixConverter().fromJson(args['pose']));
          }
          break;
